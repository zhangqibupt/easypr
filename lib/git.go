...
func generatePRLink(source, target string, orignalPRLink string) (string, error) {
	...
	if isCherryPick {
		description := url.QueryEscape(fmt.Sprintf("Orignal PR is [Link](%s)\nCherryPick Pull Requests Auto Generated by [fwpr](https://github.freewheel.tv/qzhang/fwpr)", orignalPRLink))
		fullURL = fmt.Sprintf("%s&title=%s&body=%s", fullURL, url.QueryEscape("CherryPick#"+source), description)
	}
